...
//  FindMeAHotSpot
//
//  Created by Mohammad Farzane on 2019-03-13.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        default:
            ...
            (nav?.topViewController as? RestTableTableViewController)?.delegete = self
        }
        ...
        service.request(.search(lat: coordinate.latitude, long: coordinate.longitude)) { [weak self] (result) in
            ...
            case .success(let response):
                ...
                if let nav = strongSelf.window.rootViewController as? UINavigationController,
                    let restaurantListViewController = nav.topViewController as? RestTableTableViewController {
                    ...
                    strongSelf.window.rootViewController?.present(nav, animated: true) {
                        (nav.topViewController as? RestTableTableViewController)?.delegete = self
                        (nav.topViewController as? RestTableTableViewController)?.viewModels = viewModels ?? []
                    }
                }
            case .failure(let error):
                print("Error: \(error)")
